...
if (isset($_POST['fname'])           &&
    ...
    if (empty($fname)) {
        ...
        header("Location: ../admin/teacher-add.php?error=$em&$data");
        exit;
    }else if (empty($lname)) {
        ...
        $sql = "INSERT INTO
                teacher(username, password, class, fname, lname, subject, address, employee_number,date_of_birth,
                        phone_number, qualification, gender, email_address)
                VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)"; //13

        $stmt = $conn->prepare($sql);
        $stmt->execute([$uname, $pass, $class, $fname, $lname, $subject, $address, $employee_number, $date_of_birth,
                        $phone_number, $qualification, $gender, $email_address]);
        $sm = "New teacher registered successfully!";
        header("Location: ../admin/teacher-add.php?success=$sm");
        exit;
    }

    }else {
        $em = "Error occured";
        header("Location: ../admin/teacher-add.php?error=$em");
        exit;
    } 
    }else {
