...
if (isset($_POST['score-1'])      &&
    ...
    if (empty($score_1) || empty($score_2) || empty($score_3) || empty($score_4) || empty($score_5)
    || empty($aoutof_1) || empty($aoutof_2) || empty($aoutof_3) || empty($aoutof_4) || empty($aoutof_5)
    || empty($student_id) || empty($subject_id) || empty($current_semester) || empty($current_year)) {
        ...
        if (empty($data)) {
            ...
            $sql = "UPDATE student_score SET
                results = ?
                WHERE semester=? AND year=? AND student_id=? AND teacher_id=? AND subject_id=?";
        ...
        exit;
                ...
                $sql = "INSERT INTO student_score (semester, year, student_id, teacher_id, subject_id, results)
                        VALUES(?,?,?,?,?,?)";
        $stmt = $conn->prepare($sql);
